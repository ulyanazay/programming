...
public:
    string size;
    int numberOfLegs;
    Zebra(const string& name, const int& age) : Animal(name, age) {}
...
public:
    string lenghtOfNeck;
    int numberOfSpots;
    Giraffe(const string& name, const int& age) : Animal(name, age) {}
...
public:
    bool canFly;
    string shadeOfPink;
    Flamingo(const string& name, const int& age) : Animal(name, age) {}
...
public:
    bool isHeavy;
    int weight;
    Hipopotam(const string& name, const int& age) : Animal(name, age) {}
...
int main() {
    ...
    Giraffe giraffe("Melman", 27);
    giraffe.lenghtOfNeck = "long";
    giraffe.numberOfSpots = 172;
    giraffe.swim();
    giraffe.makeSound();
    giraffe.hide();
    ...
    Flamingo flamingo("Mike", 34);
    flamingo.canFly = true;
    flamingo.shadeOfPink = "dark";
    flamingo.swim();
    flamingo.makeSound();
    flamingo.layEggs();
    flamingo.tryToFly();
    ...
    Hipopotam hipopotam("Gloria", 25);
    hipopotam.isHeavy = true;
    hipopotam.weight = 82;
    hipopotam.eat();
    hipopotam.makeSound();
    hipopotam.jump();
